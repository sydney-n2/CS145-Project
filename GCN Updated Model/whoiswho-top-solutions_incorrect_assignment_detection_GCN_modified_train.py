            ...
            if args.test_dir is not None:
                ...
                with torch.no_grad():
                    for tmp_test in tqdm(test_data):
                        ...
                        logit = encoder(node_outputs, adj_matrix)
                        logit = logit.squeeze(-1)
                        probs = F.sigmoid(logit).detach().cpu().numpy()  # Apply sigmoid and convert to numpy

                        if author_id not in result:
                            result[author_id] = {}
                        for i in range(len(pub_id)):
                            result[author_id][pub_id[i]] = float(probs[i])  # Store probabilities instead of logits

                with open(f'{args.saved_dir}/ind_valid_author_submit.json', 'w') as f:
                    json.dump(result, f)